...
///////----------------------UDP SOCKET CONNECTION ADAPATED FROM BEEJUS---------------------------------------------/////////////////////
	...
	udp_serv_addr.sin_port = htons(serverB_UDP_PORT);

	int bind_to_socket = bind(udp_socket, (struct sockaddr*)&udp_serv_addr, sizeof(udp_serv_addr));
	if(bind_to_socket<0){
		perror("Server B socket can't be bound to");
		close(udp_socket);
		return 0;
	}
	else {
		cout<<"The ServerB is up and running using UDP on port <"<<serverB_UDP_PORT<<">"<<endl;
	}
	...
	while(true)
	{
		...
		word = message;
		cout<<"The ServerB received input <"<<word<<"> and operation <"<<input_function<<">"<<endl;
		if (input_function.compare(pre_func)==0)
		{
			...
			cout<<"The ServerB has found < "<<pre_out_data.size()-1<<" > matches"<<endl;
		...
		else if (input_function.compare(search_func)==0)
		{
			...
			if (outData.compare("") == 0){
				output = "The ServerB has found < 0 > matches and ";
			}
			else {
				output = "The ServerB has found < 1 > matches and ";
			}
		...
		else if (input_function.compare(suf_func)==0)
		{
			...
			cout<<"The ServerB has found < "<<suf_out_data.size()-1<<" > matches"<<endl;
		...
		cout<<"The ServerB finished sending the output to AWS"<<endl;
